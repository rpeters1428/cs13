...
public class BetterArray<T> {
	...
	// Reduce length
	public void remove() { // If the last element in the array "length" doesn't
							// equal null
		if (arr[length - 1] != null) {
			arr[length] = null;
			length--;
		} else { // Else throw an exception showing that the "length" is out of
					// bounds
	...
	// Insert String at given index and move all Strings one slot over to the
	// right
	public void insert(int index, T s) {
		...
		} else // If the index is over the "length" then we need to extend the
				// array
		{
	...
	public void delete(int index) {
		if (index < length && index >= 0) {
			...
			length--;
		} else // Again if "length" is too high or under 0 (negative) then throw
				// exception
		{
			throw new java.lang.ArrayIndexOutOfBoundsException();
		}
